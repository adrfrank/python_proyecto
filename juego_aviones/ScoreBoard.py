...
class LivesLabel(SpriteText):
	...
	def update(self, *args):
		...
		pass
	pass
class GameOverLabel(SpriteText):
	def __init__ (self, width, height):
		SpriteText.__init__(self, width, height)		
		self.color = colors["red"]
		self.font = pygame.font.SysFont("", 40)
		self.setPosition((width)/2,(height/2))
		self.setText("Juego Terminado")
		self.setPosition((width-self.rect.w)/2,(height-self.rect.h)/2)
		self.setText("Juego Terminado")
...
class ScoreBoard():
	def __init__(self,width,height):
		...
		self.gameOverLabel = GameOverLabel(width,height)
	...
	def draw(self, scren, gamestate):
		...
		if gamestate.state == ENDED:
			self.scoreboard.add(self.gameOverLabel)
		else:
			self.scoreboard.remove(self.gameOverLabel)
		pass
